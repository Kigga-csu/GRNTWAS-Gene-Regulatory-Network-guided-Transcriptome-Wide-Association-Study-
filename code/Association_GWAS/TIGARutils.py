...
def ped_startup(ped_path, pedinfo_path):
    ...
    if len(missing_cols):
        print(
            'WARNING: The following phenotypes and/or covariates given in PED_info are missing in the PED file: ' + ', '.join(
                missing_cols) + '\n')
...
def sampleid_startup(chrm=None, genofile_type=None, data_format=None, sampleid_path=0, geno_path=0, geneexp_path=0,
                     ped_path=0, pedinfo_path=0, **kwargs):
    sampleid_lst = []
...
def read_gene_annot_exp(chrm=None, geneexp_path=None, annot_path=None,
                        cols=['CHROM', 'GeneStart', 'GeneEnd', 'TargetID', 'GeneName'], col_inds=[0, 1, 2, 3, 4],
                        dtype={'CHROM': object, 'GeneStart': np.int64, 'GeneEnd': np.int64, 'TargetID': object,
                               'GeneName': object}, **kwargs):
    # set the path
...
def read_tabix(start, end, sampleID, chrm, path, file_cols, col_inds, cols, dtype, genofile_type=None, data_format=None,
               target_ind=5, target=None, weight_threshold=0, raise_error=True, **kwargs):
    # subprocess command
    ...
    if (genofile_type == 'weight'):
        ...
        if weight_threshold:
            ...
            if df.empty and raise_error:
                print(
                    'No test SNPs with cis-eQTL weights with magnitude that exceeds specified weight threshold for TargetID: ' + target + '.\n')
                raise NoTargetDataError
...
def call_tabix_header(path, out='tuple', rename={}):
    ...
    try:
        header_df = pd.read_csv(
            StringIO(proc_out.decode('utf-8', errors='ignore')), #  errors='ignore' 
            sep='\t',
            on_bad_lines='skip' # 
        ).rename(columns=rename)

    except pd.errors.EmptyDataError:
        print(f":  '{path}'  tabix -H 。")
        if out == 'tuple':
            return tuple()
        elif out == 'list':
            return []
    ...
    if out == 'tuple':
        return tuple(header_df.columns) # 
    elif out == 'list':
        return list(header_df.columns) # 
    else:
        return header_df #  DataFrame
...
def compare_snpIDs_flexible_chr(snpID1, snpID2):
    if not snpID1 or not snpID2: return False
    parts1 = snpID1.split(':', 1) # 
    parts2 = snpID2.split(':', 1)
    if len(parts1) != 2 or len(parts2) != 2: return False # 

    chrom1 = parts1[0].replace('chr', '')
    chrom2 = parts2[0].replace('chr', '')
    rest1 = parts1[1]
    rest2 = parts2[1]

    return chrom1 == chrom2 and rest1 == rest2
...
def get_complement_snpID(snpID):
    """
    Generates the complement strand SNP ID for a given SNP ID (CHROM:POS:REF:ALT).
    Handles multi-base alleles (like INDELs) by complementing each base.

    Args:
        snpID (str): SNP ID in CHROM:POS:REF:ALT format. Alleles can be multi-base.

    Returns:
        str or None: The complement strand SNP ID in the same format
                     (CHROM:POS:REF_COMP:ALT_COMP), or None if the input format
                     is invalid, alleles are empty, or alleles contain
                     non-complementable bases (characters other than A, T, C, G).
    """
    if not isinstance(snpID, str): # Basic type check
        return None
    try:
        parts = snpID.split(':')
        if len(parts) != 4:
            # Invalid format (must have 4 parts)
            return None

        chrom, pos, ref, alt = parts

        # Ensure REF and ALT are not empty strings
        if not ref or not alt:
            return None

        ref_comp_chars = []
        for base in ref.upper(): # Iterate through REF bases (convert to upper for lookup)
            comp = complement_map.get(base)
            if comp is None:
                # Found a non-ACGT base (like 'N', '-', etc.), cannot complement
                return None
            ref_comp_chars.append(comp)
        ref_comp = "".join(ref_comp_chars) # Join complemented bases for REF

        alt_comp_chars = []
        for base in alt.upper(): # Iterate through ALT bases (convert to upper for lookup)
            comp = complement_map.get(base)
            if comp is None:
                # Found a non-ACGT base, cannot complement
                return None
            alt_comp_chars.append(comp)
        alt_comp = "".join(alt_comp_chars) # Join complemented bases for ALT

        # If both REF and ALT were successfully complemented, return the new ID
        return f"{chrom}:{pos}:{ref_comp}:{alt_comp}"

    except Exception as e:
        # Catch potential errors during splitting or processing
        # print(f"Warning: Could not process SNP ID '{snpID}' for complementation: {e}") # Optional warning
        return None
...
def get_complement_snpID_2(snpID):
    """
    Generates the complement strand SNP ID for a given SNP ID (CHROM:POS:REF:ALT).
    Handles multi-base alleles (like INDELs) by complementing each ATCGN base
     and retaining other characters (like '-', 'I', etc.).

    Args:
        snpID (str): SNP ID in CHROM:POS:REF:ALT format. Alleles can be multi-base.

    Returns:
        str or None: The complement strand SNP ID in the format
                     (CHROM:POS:REF_COMP:ALT_COMP).
                     Returns None if the input is not a string,
                     or the string format is severely invalid (e.g., not 4 parts separated by ':').
                     Note: Bases other than A, T, C, G, N are preserved as is.
    """
    if not isinstance(snpID, str):
        return None

    try:
        parts = snpID.rsplit(':', 3) #  3 ， 4 
...
def calculate_denominator_block(w_block, V_cov_block, test_stat):
    """Calculates w'Vw for a single LD block."""
    if test_stat == 'FUSION':
        V_cor_block = get_V_cor(V_cov_block)
        denom_sq = np.linalg.multi_dot([w_block, V_cor_block, w_block])
    elif test_stat == 'SPrediXcan':
        denom_sq = np.linalg.multi_dot([w_block, V_cov_block, w_block])
    else: # Should not happen based on arg parsing
        denom_sq = 0
    return denom_sq